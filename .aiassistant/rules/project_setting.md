...
| **외부 AI 연동**| **Google Gemini API** | `gemini-1.5-pro`, `gemini-1.5-flash` | `aiText.service.ts`: 키워드 추천 등 고품질 텍스트 생성에는 `pro`, 단순 생성에는 `flash`를 사용합니다. |
| | **Google Gemini API** | `gemini-1.5-flash` | `aiImage.service.ts`: 이미지 생성 기능이 포함된 `flash` 모델을 사용하여 채용 포스터 및 웹 배너를 생성합니다. |
